...
ALTER TABLE `marketplace_listings` ADD CONSTRAINT `mkt_listings_conn_fk` FOREIGN KEY (`marketplace_connection_id`) REFERENCES `marketplace_connections`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `sync_logs` ADD CONSTRAINT `sync_logs_conn_fk` FOREIGN KEY (`marketplace_connection_id`) REFERENCES `marketplace_connections`(`id`) ON DELETE set null ON UPDATE no action;